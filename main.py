...
def bill_button_operation():
    ...
    if ans == "yes":
        ...
        st = "\t\t\t\tWash & Fold Laundry\n\t\t\tSHOP NO. 75 1^st FLOOR,PANCHKULA SHOPPING COMPLEX, (PEER MUCHALLA,NEAR SEC.20,PKL.) \n"
        ...
        xlWorkBook = xl.Workbook("out.xlsx",)
        xlSheet = xlWorkBook.add_worksheet("Invoice")
        ...
        namesxl = lis[0]
        headerxl = ["No.", "Name", "Rate", "Quantity", "Price","Total Price:- "+totalPrice.get(),"Time:- "]
        numberxl = ["no"]
        titlexl = ["Wash & Fold Laundry"]
        adressxl = ["SHOP NO. 75 1st FLOOR,PANCHKULA SHOPPING COMPLEX, (PEER MUCHALLA,NEAR SEC.20,PKL."]
        ratexl = lis[1]
        quantityxl = lis[2]
        pricexl = lis[3]
        timexl = [t.tm_mday,t.tm_mon,t.tm_year]

        col = 0
        row = 0

        xlSheet.write(row + 3, col + 2, namesxl)
        xlSheet.write(row + 3, col + 3, ratexl)
        xlSheet.write(row + 3, col + 4, quantityxl)
        xlSheet.write(row + 3, col + 5, pricexl)
        xlSheet.write(row + 2, col + 7, timexl[0])
        xlSheet.write(row + 2, col + 8, timexl[1])
        xlSheet.write(row + 2, col + 9, timexl[2])
        # xlSheet.write(3, 0, numberxl)

        xlSheet.write(row, col + 4, titlexl[0])

        xlSheet.write(row + 1, col + 1, adressxl[0])

        xlSheet.write(row + 2, col + 1, headerxl[0])
        xlSheet.write(row + 2, col + 2, headerxl[1])
        xlSheet.write(row + 2, col + 3, headerxl[2])
        xlSheet.write(row + 2, col + 4, headerxl[3])
        xlSheet.write(row + 2, col + 5, headerxl[4])
        xlSheet.write(row + 6, col + 3, headerxl[5])
        xlSheet.write(row + 2, col + 6, headerxl[6])
        xlWorkBook.close()
...
def payment_button_operation():
    ...
    if not customerContact.get().isdigit():
        tmsg.showinfo("Error", "Invalid Customer Contact")
        return
    if not customerContact.get().isdigit() > 10 & customerContact.get().isdigit() < 10:
        tmsg.showinfo("Error", "Invalid Customer Contact")
        return
    ans = tmsg.askquestion("Generate Bill", "Are You Sure to Generate Bill?")
    ans = "yes"
    if ans == "yes":
        ...
        bill.wm_iconbitmap("Coffee.ico")
        bill_text_area = Text(bill, font=("arial", 12))
        st = "\t\t\t\tWash & Fold Laundry\n\t\t\tSHOP NO. 75 1st FLOOR,PANCHKULA SHOPPING COMPLEX, (PEER MUCHALLA,NEAR SEC.20,PKL.) \n"
        st += "\t\t\tGST.NO:- 27AHXPP3379HIZH\n"
        st += "-" * 61 + "BILL" + "-" * 61 + "\nDate:- "
        ...
        # write into file
        folder = f"{t.tm_mday},{t.tm_mon},{t.tm_year}"
        if not os.path.exists(f"Payment Records\\{folder}"):
            os.makedirs(f"Payment Records\\{folder}")
        file = open(f"Payment Records\\{folder}\\{customer_name + customer_contact}.txt", "w")
        file.write(st)
        file.close()
...
def deliver_button_operation():
    ...
    if ans == "yes":
        ...
        # write into file
        folder = f"{t.tm_mday},{t.tm_mon},{t.tm_year}"
        if not os.path.exists(f"Delivery Records\\{folder}"):
            os.makedirs(f"Delivery Records\\{folder}")
        file = open(f"Delivery Records\\{folder}\\{customer_name + customer_contact}.txt", "w")
        file.write(st)
        file.close()
...
root.title("Welcome to Wash & Fold Laundry")
...
title_label = Label(title_frame, text="Wash & Fold Laundry",
...
item_number_label = Label(item_frame2, text="Name",
                        font=("arial", 12, "bold"), bg="lightgreen", fg="blue")
item_number_label.grid(row=0, column=0)

item_number_label = Label(item_frame2, text="No.",
                        font=("arial", 12, "bold"), bg="lightgreen", fg="blue")
item_number_label.grid(row=1, column=2)
...
itemNumber = IntVar()
itemNumber.set(0)
item_number = Entry(item_frame2, font="arial 12", textvariable=itemNumber,state=DISABLED, width=10)
item_number.grid(row=1, column=3)
...
payment_button = ttk.Button(order_frame, text="Payment", width=8,
                            command=payment_button_operation)
payment_button.pack(side=LEFT, anchor=SW, padx=0, pady=10)

deliver_button = ttk.Button(order_frame, text="Delivered", width=9,
                            command=deliver_button_operation)
deliver_button.pack(side=LEFT, anchor=SW, padx=10, pady=10)

print_button = ttk.Button(order_frame, text="Delivered", width=9,
                          command=print_button_operation)
print_button.pack(side=LEFT, anchor=SW, padx=10, pady=10)

cancel_button = ttk.Button(order_frame, text="Cancel Order", command=cancel_button_operation)
cancel_button.pack(side=LEFT, anchor=SW, padx=1, pady=10)
